...
| M8 | **Device-aware animation thumbnails** — `AnimThumbnail` currently renders an iPhone CSS shape for all devices. Mac → show a CSS laptop shape (screen lid + keyboard base, landscape). iPad → wider portrait rectangle. Watch → square rounded body. Multi-device scene → abstract silhouette of 2–3 small overlapping rectangles. `AnimSection` and `RightPanel` pass `deviceCategory` down; `SceneEditor` passes `'scene'`. | Every animation card shows an iPhone even when editing a MacBook — visually wrong and confusing for non-designers. |
| M9 | **MacBook material improvements** — anodized aluminum is brushed/satin, not polished. Currently every device uses the same `roughness: 0.22, metalness: 0.88`. Mac body meshes should use `MeshPhysicalMaterial` with `roughness: 0.30, metalness: 0.95` (closer to real aluminum). Keyboard/key meshes get higher roughness (~0.55) and lower metalness (~0.65). Trackpad gets polished glass treatment (`roughness: 0.18`). Pass `category` into `DeviceModel` and branch the material logic per-category. | HDRI environment barely shows on Macs because the surface is too polished and dark — correct material values make the brushed aluminum look realistic under IBL. |
